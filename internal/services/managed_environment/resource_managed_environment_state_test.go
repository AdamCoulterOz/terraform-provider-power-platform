...
func TestPopulateStateForEnvironmentGroup_WhenPlanContainsUnknowns_DoesNotLeaveUnknowns(t *testing.T) {
	plan := &ManagedEnvironmentResourceModel{
		Id:                                     types.StringUnknown(),
		EnvironmentId:                          types.StringValue("00000000-0000-0000-0000-000000000001"),
		ProtectionLevel:                        types.StringUnknown(),
		IsUsageInsightsDisabled:                types.BoolUnknown(),
		IsGroupSharingDisabled:                 types.BoolUnknown(),
		MaxLimitUserSharing:                    types.Int64Unknown(),
		LimitSharingMode:                       types.StringUnknown(),
		SolutionCheckerMode:                    types.StringUnknown(),
		SuppressValidationEmails:               types.BoolUnknown(),
		SolutionCheckerRuleOverrides:           types.SetUnknown(types.StringType),
		PowerAutomateIsSharingDisabled:         types.BoolUnknown(),
		CopilotAllowGrantPermissionsWhenShared: types.BoolUnknown(),
		CopilotLimitSharingMode:                types.StringUnknown(),
	...
	env := &environment.EnvironmentDto{
		Properties: &environment.EnviromentPropertiesDto{
			ParentEnvironmentGroup: &environment.ParentEnvironmentGroupDto{
	...
	var diagnostics diag.Diagnostics
	resource := &ManagedEnvironmentResource{}
	resource.populateStateForEnvironmentGroup(context.Background(), plan, env, &diagnostics)

	if diagnostics.HasError() {
		t.Fatalf("populateStateForEnvironmentGroup returned unexpected diagnostics: %v", diagnostics)
	}
